...
impl<'a> ChannelsTabs {
    ...
    pub fn activated(&'a self) -> Vec<(usize, &'a str)> {
        self.activated.iter().map(|idx| {
            (*idx, self.titles[*idx].as_str())
        }).collect()
    }

    fn states(&'a self) -> Vec<(bool, &'a str)> {
        (0..self.count()).into_iter().map(|idx| {
    ...
    fn get_channels_titles(count: usize) -> Vec<String> {
        match count {
            ...
            3_usize => vec!["L", "R", "LFE"].into_iter().map(|v| v.to_string()).collect(),                           // 2.1
            5_usize => vec!["FL", "FR", "C", "BL", "BR"].into_iter().map(|v| v.to_string()).collect(),               // 5.0
            6_usize => vec!["FL", "FR", "C", "LFE", "BL", "BR"].into_iter().map(|v| v.to_string()).collect(),        // 5.1
            7_usize => vec!["FL", "FR", "C", "LFE", "SL", "SR", "BC"].into_iter().map(|v| v.to_string()).collect(),  // 6.1
            8_usize => vec!["FL", "FR", "C", "LFE", "SL", "SR", "BL", "BR"].into_iter().map(|v| v.to_string()).collect(),  // 7.1
            _ => (0..count).into_iter().map(|idx| format!["Channel {:?}", idx]).collect()
        }
    }
}
